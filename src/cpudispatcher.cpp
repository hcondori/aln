...
void aln::CPUDispatcher::run(float gap_open, float gap_extend, float* sm, float table_fraction)
{
  ...
  for(int i = 0; i < nthreads; i++)
    threads.push_back(std::thread([this, nthreads, gap_open, gap_extend, sm, table_fraction]() 
      {        
                             ...
                             float mismatch, float table_fraction)
{
  ...
  std::vector<std::thread> threads;
  for(int i =0;i< nthreads;i++)
    threads.push_back(std::thread([this, gap_open, gap_extend, match, mismatch, table_fraction]() 
      {
        float band_offset = sqrt(table_fraction);
        int c;
        int pair_count = 0;
    ...
    threads.push_back(std::thread([this, gap_open, gap_extend, match, mismatch, table_fraction]() 
      {
        ...
        int c;
        int pair_count = 0;
        int16_t* aF = (int16_t*) aligned_alloc (32, 16 * 256 * sizeof(int16_t));
        int16_t* aH = (int16_t*) aligned_alloc (32, 16 * 256 * sizeof(int16_t));
        //int* flags = (int*) malloc (256 * 256 * sizeof(int));
        int16_t ipos[16] __attribute__ ((aligned (32)));
        int16_t jpos[16] __attribute__ ((aligned (32)));
        char aln1[2*256+1];
        char aln2[2*256+1];
        int16_t x0, y0;
